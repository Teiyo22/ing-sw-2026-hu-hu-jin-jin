...
public class StateInfoSerializer implements JsonSerializer<ModelStateInfo> {
    ...
    public JsonElement serialize(ModelStateInfo src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        return switch (type) {
            case CARD_PICK -> context.serialize(src, CardPickStateInfo.class);
            case OFFER_PICK -> context.serialize(src, OfferPickStateInfo.class);
            case GAME_END -> context.serialize(src, GameEndStateInfo.class);
        };
    }
}
